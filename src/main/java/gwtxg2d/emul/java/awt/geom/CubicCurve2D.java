...
import gwtxg2d.client.helper.Arrays;
...
public abstract class CubicCurve2D implements Shape, Cloneable {
  ...
  public static int solveCubic(double eqn[], double res[]) {
    ...
    if (D < 0) { /* Casus irreducibilis: three real solutions */
      ...
      double err = 1200000000 * gwtxg2d.client.helper.Math.ulp(abs(uv) + abs(sub));
  ...
  private static int fixRoots(double[] eqn, double[] res, int num) {
    ...
    if (num == 3) {
      ...
      if (iszero(fx, 10000000 * gwtxg2d.client.helper.Math.ulp(x))) {
  ...
  private static double refineRootWithHint(double[] eqn, double min, double max, double t) {
    ...
    if (within(t, origt, 1000 * gwtxg2d.client.helper.Math.ulp(origt)) && inInterval(t, min, max)) {
      return t;
    }
    return origt;
  }
  ...
  private static double getRootUpperBound(double[] eqn) {
    ...
    M += gwtxg2d.client.helper.Math.ulp(M) + 1;
    return M;
  }
